...
public class AyudaGastosFunerarios {
	...
	public DatosRequest datosAsegurado(DatosRequest request, Integer idFinado) throws UnsupportedEncodingException {
		...
		query.append("DATE_FORMAT(agf.FEC_DEFUNCION,'%Y%m%d') AS fechaDefuncion, CONVERT(vel.ID_DELEGACION,CHAR) AS delegacion, \n");
		query.append("agf.ID_VELATORIO AS velatorioOperador, per.NUM_SEXO AS sexo, \n");
	...
	public DatosRequest datosInteresado(DatosRequest request, Integer idFinado) throws UnsupportedEncodingException {
		...
		query.append("cben.ID_PARENTESCO AS parentesco, DATE_FORMAT(os.FEC_ALTA,'%Y%m%d') AS fechaSolicitud \n");
